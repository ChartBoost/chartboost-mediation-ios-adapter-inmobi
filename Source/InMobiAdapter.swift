...
final class InMobiAdapter: NSObject, PartnerAdapter {
    ...
    var configuration: PartnerAdapterConfiguration.Type { InMobiAdapterConfiguration.self }
    ...
    func fetchBidderInformation(request: PartnerAdPreBidRequest, completion: @escaping (Result<[String: String], Error>) -> Void) {
        ...
        let extras = ["tp": "c_chartboost", "tp-ver": ChartboostMediation.sdkVersion]
        let bidToken = IMSdk.getTokenWithExtras(extras, andKeywords: nil)
        log(.fetchBidderInfoSucceeded(request))
        completion(.success(bidToken.map { ["token": $0] } ?? [:]))
    }
    ...
    func setConsents(_ consents: [ConsentKey: ConsentValue], modifiedKeys: Set<ConsentKey>) {
        ...
        if let applies = UserDefaults.standard.string(forKey: .tcfGDPRApplies) {
            ...
            value[IMCommonConstants.IM_PARTNER_GDPR_APPLIES] = "0"
        }
...
extension PartnerConfiguration {
    fileprivate var accountID: String? { credentials[.accountIDKey] as? String }
}

extension String {
    /// InMobi account ID credentials key
    fileprivate static let accountIDKey = "account_id"
    /// InMobi GDPR available consent value. Defined in IMSdk.h comments.
    fileprivate static let gdprConsentAvailable = "true"
    /// InMobi GDPR unavailable consent value. Defined in IMSdk.h comments.
    fileprivate static let gdprConsentUnavailable = "false"
    ...
    fileprivate static let tcfGDPRApplies = "IABTCF_gdprApplies"
}
